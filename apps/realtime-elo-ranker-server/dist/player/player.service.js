...
let PlayerService = class PlayerService {
    playerRepository;
    eloService;
    constructor(playerRepository, eloService) {
    ...
    async create(createPlayerDto) {
        ...
        const initialElo = players.length > 0
            ? Math.round(players.reduce((acc, p) => acc + p.elo, 0) / players.length)
            : 1200;
    ...
    async findOne(id) {
        ...
        if (!player)
            throw new common_1.NotFoundException(`Joueur '${id}' non trouvé`);
        return player;
    }
    async update(id, _updatePlayerDto) {
